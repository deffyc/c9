...
RUN sudo -S echo "$user:$pass" | sudo chpasswd
USER $user
WORKDIR /home/$user
...
RUN sudo apt update && apt install -y python-dev
RUN sudo mkdir /tmp/codeintel && pip install --download /tmp/codeintel codeintel==0.9.3
